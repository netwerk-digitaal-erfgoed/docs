...
If you already run a public SPARQL endpoint, this is the route we recommend. The description lives next to the data it describes, you edit it with the same tools, and the Register sees the change the next day. The one thing to watch is that the registration URL is tied to your endpoint: if you move to another endpoint, register the new URL. The dataset URIs inside the description must stay the same, but that holds for every route.
